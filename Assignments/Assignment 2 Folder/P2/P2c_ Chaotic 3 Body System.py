...
initial_diff_12 = alg.norm(np.subtract(positions_2[:,0], positions_1[:,0]))
initial_diff_13 = alg.norm(np.subtract(positions_3[:,0], positions_1[:,0]))
initial_diff_23 = alg.norm(np.subtract(positions_3[:,0], positions_2[:,0]))
initial_diff_21 = -initial_diff_12
initial_diff_31 = -initial_diff_13
initial_diff_32 = -initial_diff_23
...
# Potential Energies of 1, 2, and 3, these neededd to be updated iteratively since it depends 

Potential_Energy_1 = [-constant_m1*(m2/initial_diff_12+m3/initial_diff_13)]
Potential_Energy_2 = [-constant_m2*(m1/initial_diff_21+m3/initial_diff_23)]
Potential_Energy_3 = [-constant_m3*(m1/initial_diff_31+m2/initial_diff_32)]
Kinetic_Energy_1 = [1/2*m1*((alg.norm(velocities_1[:,0]))**2)]
Kinetic_Energy_2 = [1/2*m2*((alg.norm(velocities_2[:,0]))**2)]
Kinetic_Energy_3 = [1/2*m3*((alg.norm(velocities_3[:,0]))**2)]
...
for i in time_array:
	...
	curr_diff_12 = alg.norm(np.subtract(new_pos_2, new_pos_1))
	curr_diff_13 = alg.norm(np.subtract(new_pos_3, new_pos_1))
	curr_diff_23 = alg.norm(np.subtract(new_pos_3, new_pos_2))
	curr_diff_21 = -curr_diff_12
	curr_diff_31 = -curr_diff_13
	curr_diff_32 = -curr_diff_23
	Potential_Energy_1.append(-constant_m1*(m2/curr_diff_12+m3/curr_diff_13))
	Potential_Energy_2.append(-constant_m2*(m1/curr_diff_21+m3/curr_diff_23))
	Potential_Energy_3.append(-constant_m3*(m1/curr_diff_31+m2/curr_diff_32))
	Kinetic_Energy_1.append(1/2*m1*(alg.norm(new_vel_1)**2))
	Kinetic_Energy_2.append(1/2*m2*(alg.norm(new_vel_2)**2))
	Kinetic_Energy_3.append(1/2*m3*(alg.norm(new_vel_3)**2))

Total_Energy = np.asarray(Kinetic_Energy_1[1:]) + np.asarray(Potential_Energy_1[1:])+np.asarray(Kinetic_Energy_2[1:])+np.asarray(Potential_Energy_2[1:])+np.asarray(Kinetic_Energy_3[1:])+np.asarray(Potential_Energy_3[1:])

fig_1 = plt.figure()

te_line = plt.plot(time_array, Total_Energy, label="Total Energy of the system")
plt.legend()
plt.title("Figure 1d: Total Energy of the System", font = cm)
plt.xlabel("Time (s)", font = cm)
plt.ylabel("Total Energy of the System (J)", font = cm)
plt.savefig("Figure 2c.svg")
