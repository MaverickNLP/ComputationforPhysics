# This code is adapted from my code for a system with three bodies so the set up isn't what is demanded from us by the question doc.
# If they "collide" then the gravitational force approaches infinity, forcing body 2 and body 3 apart, it is conceivable that they would approach each other after a very long time, but I'm not sure whether it's worth simulating that. 
import numpy as np
...
positions_2[0:3,0] = [1, 0, 0]
positions_3[0:3,0] = [-1, 0, 0]
...
velocities_2[0:3,0] = [0, 0.5, 0]
velocities_3[0:3,0] = [0, -0.5, 0]
...
# Plotting - for the momentum plot.
fig_2 = plt.figure()
plt.title("Figure 2a: The Momenta of Both Bodies", font = cm)
plt.xlabel("Time (s)", font = cm)
plt.ylabel("Speed (kgm/s)", font = cm)
plt.plot(time_array, momentum_2[0, 1:]+momentum_2[1, 1:], label = "Momentum of Body 2")
plt.plot(time_array, momentum_3[0, 1:]+momentum_3[1, 1:], label = "Momentum of Body 3")
plt.plot(time_array, momentum_2[0, 1:]+momentum_3[0, 1:]+momentum_3[1, 1:]+momentum_2[1, 1:], label = "Total Momentum of System")
plt.legend()
plt.savefig("Figure 2.svg")
plt.show()

# Plotting for the gif of the bodies orbiting each other stably. 
...
fig.legend()
ax.set_title("Figure 2: Stable System of Two Bodies Orbiting Each Other", font = cm)
...
def update(frame):
	...
	line_2 = plot_mass_2_1.set_data_3d(positions_2[0,cur_frame_ind-3000:cur_frame_ind], positions_2[1,cur_frame_ind-3000:cur_frame_ind],positions_2[2,cur_frame_ind-3000:cur_frame_ind])
	line_3 = plot_mass_3_1.set_data_3d(positions_3[0,cur_frame_ind-3000:cur_frame_ind], positions_3[1,cur_frame_ind-3000:cur_frame_ind],positions_3[2,cur_frame_ind-3000:cur_frame_ind])
	return line_2, line_3, point_2, point_3

ani = animation.FuncAnimation(fig, update,frames)


ani.save("Q2.gif", fps = 120)
